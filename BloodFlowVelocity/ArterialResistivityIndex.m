function [] = ArterialResistivityIndex(signal, systolesIndexes, name, signal_se, opt)

arguments
    signal
    systolesIndexes
    name
    signal_se = []
    opt.ForceFigure = false
end

ToolBox = getGlobalToolBox;
params = ToolBox.getParams;
saveFigures = params.saveFigures;
numInterp = 60;
t = ToolBox.Cache.t;

if contains(name, 'v_')
    unit = 'mm/s';
    y_label = 'Velocity (mm/s)';
else
    unit = 'µL/min';
    y_label = 'Flow Rate (µL/min)';
end

% Color Maps

signal = double(signal);

[interp_signal, ~] = interpSignal(signal, systolesIndexes, numInterp);

% Compute mean and standard deviation of vSys and vDias
[~, amin] = min(interp_signal);
[~, amax] = max(interp_signal);

vMax = interp_signal(amax);
vMin = interp_signal(amin);

v_mean = mean(interp_signal);

% Compute RI and its uncertainty using error propagation
RI = (vMax - vMin) / vMax;

if ~isempty(signal_se)
    vMax_se = signal_se(amax);
    vMin_se = signal_se(amin);
    dRI_dvMax = vMin / (vMax ^ 2);
    dRI_dvMin = -1 / vMax;
    RI_se = sqrt((dRI_dvMax * vMax_se) ^ 2 + (dRI_dvMin * vMin_se) ^ 2);
else
    vMax_se = 0;
    vMin_se = 0;
    RI_se = 0;
end

% PI calculation
PI = (vMax - vMin) / v_mean;

if ~isempty(signal_se)
    dPI_dvMax = 1 / v_mean;
    dPI_dvMin = -1 / v_mean;
    dPI_dv_mean =- (vMax - vMin) / (v_mean ^ 2);
    PI_se = sqrt((dPI_dvMax * vMax_se) ^ 2 + (dPI_dvMin * vMin_se) ^ 2 + (dPI_dv_mean * std(interp_signal)) ^ 2);
else
    PI_se = 0;
end

% PR (Pulse Ratio) calculation
PR = vMax / vMin;

if ~isempty(signal_se)
    dPR_dvMax = 1 / vMin;
    dPR_dvMin = -vMax / (vMin ^ 2);
    PR_se = sqrt((dPR_dvMax * vMax_se) ^ 2 + (dPR_dvMin * vMin_se) ^ 2);
else
    PR_se = 0;
end

% Save figures
if saveFigures || opt.ForceFigure

    % RI Graph
    figure('Visible', 'off');
    hold on;

    if ~isempty(signal_se)
        Color_std = [0.7, 0.7, 0.7];
        curve1 = signal + signal_se;
        curve2 = signal - signal_se;
        tmp_t = [t, fliplr(t)];
        inBetween = [curve1, fliplr(curve2)];
        fill(tmp_t, inBetween, Color_std);
        plot(t, curve1, "Color", Color_std, 'LineWidth', 2);
        plot(t, curve2, "Color", Color_std, 'LineWidth', 2);
    end

    % Plot the velocity signal
    plot(t, signal, '-k', 'LineWidth', 2);

    % Add reference lines
    yline(vMax, '--k', sprintf('%.1f %s', vMax, unit), 'LineWidth', 2, ...
        'Color', [0.5 0.5 0.5], 'LabelVerticalAlignment', 'top', 'FontWeight', 'bold');
    yline(vMin, '--k', sprintf('%.1f %s', vMin, unit), 'LineWidth', 2, ...
        'Color', [0.5 0.5 0.5], 'LabelVerticalAlignment', 'bottom', 'FontWeight', 'bold');

    axis padded
    axP = axis;
    axis tight
    axT = axis;
    axis([axT(1), axT(2), - 2, axP(4) + 2])

    box on
    set(gca, 'LineWidth', 2);
    set(gca, 'PlotBoxAspectRatio', [1.618, 1, 1])
    ax = gca;
    ax.LineStyleOrderIndex = 1; % Reset if needed
    ax.SortMethod = 'depth'; % Try changing sorting method
    ax.Layer = 'top'; % This may help in some cases

    % Add a text label with white background at the right edge of the plot
    ax = gca;
    y1 = ax.YLim(1);
    y2 = ax.YLim(2);
    x1 = ax.XLim(1);
    x2 = ax.XLim(2);
    dx = x2 - x1; % Right edge of the plot
    dy = y2 - y1;

    if ~isempty(signal_se)
        text(0.35 * dx, y1 + 0.85 * dy, sprintf("RI = %0.2f ± %0.2f", RI, RI_se), ...
            'BackgroundColor', 'w', ...
            'HorizontalAlignment', 'left', ...
            'VerticalAlignment', 'middle', ...
            'FontWeight', 'bold', ...
            'FontSize', 12, ...
            'Margin', 1); % Small padding
    else
        text(0.4 * dx, y1 + 0.85 * dy, sprintf("RI = %0.2f", RI), ...
            'BackgroundColor', 'w', ...
            'HorizontalAlignment', 'left', ...
            'VerticalAlignment', 'middle', ...
            'FontWeight', 'bold', ...
            'FontSize', 12, ...
            'Margin', 1); % Small padding
    end

    xlabel('Time (s)');
    ylabel(y_label);

    % Export
    exportgraphics(gcf, fullfile(ToolBox.path_png, sprintf("%s_RI_%s.png", ToolBox.folder_name, name)));
    exportgraphics(gcf, fullfile(ToolBox.path_eps, sprintf("%s_RI_%s.eps", ToolBox.folder_name, name)));
    close;
end

if saveFigures
    % PI Graph
    figure('Visible', 'off');
    hold on;

    if ~isempty(signal_se)
        Color_std = [0.7, 0.7, 0.7];
        curve1 = signal + signal_se;
        curve2 = signal - signal_se;
        tmp_t = [t, fliplr(t)];
        inBetween = [curve1, fliplr(curve2)];
        fill(tmp_t, inBetween, Color_std);
        plot(t, curve1, "Color", Color_std, 'LineWidth', 2);
        plot(t, curve2, "Color", Color_std, 'LineWidth', 2);
    end

    % Plot the velocity signal
    plot(t, signal, '-k', 'LineWidth', 2);

    % Add reference lines
    yline(vMax, '--k', sprintf('%.1f %s', vMax, unit), 'LineWidth', 2, 'Color', [0.5 0.5 0.5], 'LabelVerticalAlignment', 'top', 'FontWeight', 'bold');
    yline(v_mean, '--k', sprintf('%.1f %s', v_mean, unit), 'LineWidth', 2, 'Color', [0.5 0.5 0.5], 'FontWeight', 'bold');
    yline(vMin, '--k', sprintf('%.1f %s', vMin, unit), 'LineWidth', 2, 'Color', [0.5 0.5 0.5], 'LabelVerticalAlignment', 'bottom', 'FontWeight', 'bold');

    axis padded
    axP = axis;
    axis tight
    axT = axis;
    axis([axT(1), axT(2), - 2, axP(4) + 2])

    box on
    set(gca, 'Linewidth', 2)
    set(gca, 'PlotBoxAspectRatio', [1.618, 1, 1])

    % Add a text label with white background at the right edge of the plot
    ax = gca;
    y1 = ax.YLim(1);
    y2 = ax.YLim(2);
    x1 = ax.XLim(1);
    x2 = ax.XLim(2);
    dx = x2 - x1; % Right edge of the plot
    dy = y2 - y1;

    if ~isempty(signal_se)
        text(0.4 * dx, y1 + 0.85 * dy, sprintf("PI = %.2f ± %.2f", PI, PI_se), ...
            'BackgroundColor', 'w', ...
            'HorizontalAlignment', 'left', ...
            'VerticalAlignment', 'middle', ...
            'FontWeight', 'bold', ...
            'FontSize', 12, ...
            'Margin', 1); % Small padding
    else
        text(0.4 * dx, y1 + 0.85 * dy, sprintf("PI = %0.2f", PI), ...
            'BackgroundColor', 'w', ...
            'HorizontalAlignment', 'left', ...
            'VerticalAlignment', 'middle', ...
            'FontWeight', 'bold', ...
            'FontSize', 12, ...
            'Margin', 1); % Small padding
    end

    xlabel('Time (s)');
    ylabel(y_label);

    % Export
    exportgraphics(gcf, fullfile(ToolBox.path_png, sprintf("%s_PI_%s.png", ToolBox.folder_name, name)));
    exportgraphics(gcf, fullfile(ToolBox.path_eps, sprintf("%s_PI_%s.eps", ToolBox.folder_name, name)));
    close;
end

% Save json

if contains(name, 'v_')

    if contains(name, 'vein')
        ToolBox.Output.DimOut.add('VeinVelocityMean', v_mean, ["example", "desc"], unit, std(interp_signal));
        ToolBox.Output.DimOut.add('VeinVelocityMax', vMax, ["example", "desc"], unit, vMax_se);
        ToolBox.Output.DimOut.add('VeinVelocityMin', vMin, ["example", "desc"], unit, vMin_se);
    elseif contains(name, 'artery')
        ToolBox.Output.DimOut.add('ArteryVelocityMean', v_mean, ["example", "desc"], unit, std(interp_signal));
        ToolBox.Output.DimOut.add('ArteryVelocityMin', vMin, ["example", "desc"], unit, vMin_se);
        ToolBox.Output.DimOut.add('ArteryVelocityMax', vMax, ["example", "desc"], unit, vMax_se);
    end

else
    % New
    if contains(name, 'vein')
        ToolBox.Output.DimOut.add('VeinFlowRateMean', v_mean, ["example", "desc"], unit, std(interp_signal));
        ToolBox.Output.DimOut.add('VeinFlowRateMax', vMax, ["example", "desc"], unit, vMax_se);
        ToolBox.Output.DimOut.add('VeinFlowRateMin', vMin, ["example", "desc"], unit, vMin_se);
    elseif contains(name, 'artery')
        ToolBox.Output.DimOut.add('ArteryFlowRateMean', v_mean, ["example", "desc"], unit, std(interp_signal));
        ToolBox.Output.DimOut.add('ArteryFlowRateMax', vMax, ["example", "desc"], unit, vMax_se);
        ToolBox.Output.DimOut.add('ArteryFlowRateMin', vMin, ["example", "desc"], unit, vMin_se);
    end

end

if contains(name, 'v_')

    if contains(name, 'vein')
        ToolBox.Output.DimOut.add('VeinResistivityIndexVelocity', RI, ["example", "desc"], '', RI_se);
        ToolBox.Output.DimOut.add('VeinPulsatilityIndexVelocity', PI, ["example", "desc"], '', PI_se);
        ToolBox.Output.DimOut.add('VeinMaxMinRatioVelocity', PR, ["example", "desc"], '', PR_se);
    elseif contains(name, 'artery')
        ToolBox.Output.DimOut.add('ArteryResistivityIndexVelocity', RI, ["example", "desc"], '', RI_se);
        ToolBox.Output.DimOut.add('ArteryPulsatilityIndexVelocity', PI, ["example", "desc"], '', PI_se);
        ToolBox.Output.DimOut.add('ArteryMaxMinRatioVelocity', PR, ["example", "desc"], '', PR_se);
    end

else

    if contains(name, 'vein')
        ToolBox.Output.DimOut.add('VeinResistivityIndexFlowRate', RI, ["example", "desc"], '', RI_se);
        ToolBox.Output.DimOut.add('VeinPulsatilityIndexFlowRate', PI, ["example", "desc"], '', PI_se);
        ToolBox.Output.DimOut.add('VeinMaxMinRatioFlowRate', PR, ["example", "desc"], '', PR_se);
    elseif contains(name, 'artery')
        ToolBox.Output.DimOut.add('ArteryResistivityIndexFlowRate', RI, ["example", "desc"], '', RI_se);
        ToolBox.Output.DimOut.add('ArteryPulsatilityIndexFlowRate', PI, ["example", "desc"], '', PI_se);
        ToolBox.Output.DimOut.add('ArteryMaxMinRatioFlowRate', PR, ["example", "desc"], '', PR_se);
    end

end

end
