...
%% Section 2: Local Background Calculation
...
graphSignal('f_artery', folder, ...
    ...
    'yLIM', [12.1,14.25], ...
    'pbaspect', [1.618,1,1], ...
    'Title', 'Average \sigma_{f} in arteries', 'xlabel', 'Time (s)', 'ylabel', 'frequency (kHz)', ...
...
fprintf(fileID, '%s : %f (%s) \r\n', 'Mean \Delta\sigma_{f} artery', mean(f_artery) - mean(f_artery_bkg), 'kHz');
fclose(fileID);
...
if veinsAnalysis
    ...
    graphSignal('f_vein', folder, ...
        t, f_vein, '-', cBlack, ...
        t, f_vein_bkg, ':', cBlack, ...
        'Title', 'Average \sigma_{f} in veins', 'xlabel', 'Time (s)', 'ylabel', 'frequency (kHz)', ...
        'Legend', {'veins', 'local background'});

    fileID = fopen(fullfile(ToolBox.path_txt, strcat(ToolBox.main_foldername, '_', 'EF_advanced_outputs', '.txt')), 'a');
    fprintf(fileID, '%s : %f (%s) \r\n', 'Mean \Delta\sigma_{f} vein', mean(f_vein) - mean(f_vein_bkg), 'kHz');
    fclose(fileID);
    ...
    graphSignal('f_vascular', folder, ...
        ...
        'Title', 'average \sigma_{f} in Vessels', 'xlabel', 'Time (s)', 'ylabel', 'frequency (kHz)', ...
        'Legend', {'arteries', 'veins', 'Local Background'});
end

fprintf("    2. Local background calculation took %ds\n", round(toc));
...
if veinsAnalysis
    ...
    graphSignal('vessels_frequency', folder, ...
        ...
        'Title', 'average \Delta\sigma_{f} in arteries and veins', 'xlabel', 'Time (s)', 'ylabel', 'frequency (kHz)');
else
    graphSignal('arteries_frequency', folder, ...
        t, df_artery_signal, '-', cBlack, ...
        'Title', 'average \Delta\sigma_{f} in arteries', 'xlabel', 'Time (s)', 'ylabel', 'frequency (kHz)');
end
...
if veinsAnalysis
    ...
    graphSignal('vessels_velocity', folder, ...
        ...
        'Title', 'Average estimated velocity in arteries and veins', 'xlabel', 'Time (s)', 'ylabel', 'Velocity (mm/s)');
else
    graphSignal('arteries_velocity', folder, ...
        t, v_artery_signal, '-', cBlack, ...
        'Fontsize' , 18, ...
        'yLIM', [-2,22], ...
        'pbaspect', [1.618 1 1], ...
        'Title', 'Average estimated velocity in arteries', 'xlabel', 'Time (s)', 'ylabel', 'Velocity (mm/s)');
end
...
[~, ~, ~, ~, sysIdx, diasIdx] = compute_diasys(v_RMS_video, maskArterySection, 'bloodFlowVelocity');
...
if veinsAnalysis
    LocalBackground_in_vessels = mean(f_bkg, 3) .* maskVessel + ~maskVessel .* mean(f_bkg, 'all');
else
    LocalBackground_in_vessels = mean(f_bkg, 3) .* maskArtery + ~maskArtery .* mean(f_bkg, 'all');
end
createHeatmap(LocalBackground_in_vessels, 'Local Background in vessels', ...
    'Local Background RMS frequency \sigma_{f bkg} (kHz)', fullfile(ToolBox.path_png, folder, sprintf("%s_f_bkg_map.png", ToolBox.main_foldername)));

% Delta f in vessels
in_vessels = mean(df, 3) .* maskVesselSection;
createHeatmap(in_vessels, 'Delta f in vessels', ...
    'Delta Doppler RMS frequency \Delta\sigma_{f} (kHz)', fullfile(ToolBox.path_png, folder, sprintf("%s_df_map.png", ToolBox.main_foldername)));

% Raw RMS frequency map
raw_map = squeeze(mean(f_video, 3));
createHeatmap(raw_map, 'RMS frequency map RAW', ...
    'RMS frequency \sigma_{f} (kHz)', fullfile(ToolBox.path_png, folder, sprintf("%s_f_map.png", ToolBox.main_foldername)));
...
% Generate histograms
histoVideoArtery = VelocityHistogram(v_RMS_video, maskArterySection, 'Artery', yLIM=[-55,100], Fontsize=18);
