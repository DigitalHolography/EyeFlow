...
if params.json.Mask.AVDiasysSegmentationNet
    ...
    try
        ...
        AVSegmentationNet = importNetworkFromONNX('Models\iternet5_av_diasys.onnx');
    catch
        % Fall back to the older function
        warning('off')
        AVSegmentationNet = importONNXNetwork('Models\iternet5_av_diasys.onnx');
        warning('on')
    end
...
elseif params.json.Mask.AVCorrelationSegmentationNet
    ...
    try
        % Try the newer function first
        AVSegmentationNet = importNetworkFromONNX('Models\iternet_5_av_corr.onnx');
    catch
        % Fall back to the older function
        warning('off')
        AVSegmentationNet = importONNXNetwork('Models\iternet_5_av_corr.onnx');
        warning('on')
    end
...
elseif params.json.Mask.AVCorrelationSegmentationNet && params.json.Mask.AVDiasysSegmentationNet
    ...
    try
        % Try the newer function first
        AVSegmentationNet = importNetworkFromONNX('Models\iternet5_av_corr_diasys.onnx');
    catch
        % Fall back to the older function
        warning('off')
        AVSegmentationNet = importONNXNetwork('Models\iternet5_av_corr_diasys.onnx');
        warning('on')
    end
