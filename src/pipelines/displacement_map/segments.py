...
def analyze_displacement_segments(
    ...
    *,
    ...
    for method, displacement_map in displacement_maps.items():
        ...
        maps = filled(map_shape) if retain_maps else None
        transverse_unmasked = filled(profile_shape)
        transverse_masked = filled(profile_shape)
        longitudinal_unmasked = filled(profile_shape)
        longitudinal_masked = filled(profile_shape)
        x_sum = filled(signal_shape)
        ...
        for prepared in prepare_segment_chunks(
            ...
            segment_index = (prepared.ring_index, prepared.branch_index)
            output_index = (*segment_index, prepared.frame_slice)
            vectors = _local_displacement_vectors(
                prepared.rotated,
                float(topology.rotation_degrees[segment_index]),
            )
            ...
            if maps is not None:
                maps[output_index] = vectors
            transverse_unmasked[output_index] = transverse_profiles(magnitude)
            transverse_masked[output_index] = transverse_profiles(
                magnitude,
                profile_masks[segment_index],
            )
            longitudinal_unmasked[output_index] = longitudinal_profiles(magnitude)
            longitudinal_masked[output_index] = longitudinal_profiles(
                magnitude,
                profile_masks[segment_index],
            )
