...
from ..dopplerview.constants import LEGACY_VELOCITY_SIGNAL_LOWPASS_HZ
...
def pack_hemifield_velocity_outputs(
    ...
    """Pack region-indexed velocity and per-beat velocity outputs.

    The public velocity signals are global vessel aggregates, so their
    hemifield counterparts are calculated from the spatially indexed
    cross-section segment signals.  The same branch-to-region membership used
    by :func:`pack_hemifield_metrics` is used here as well.

    ``Velocity`` publishes ``Raw`` and ``Filtered`` signals.  In
    ``VelocityPerBeat``, the existing ``BandLimited`` name is retained for
    the filtered per-beat signal.
    """
    schema = _resolve_output_paths(output_paths)
    result: dict[str, object] = {}
...
def _pack_region_velocity_outputs(
    ...
    if segment_velocity.shape[:2] != membership.shape[1:][::-1]:
        raise ValueError(
            ...
            f"{segment_velocity.shape[:2]} versus "
            f"{membership.shape[1:][::-1]}."
        )
    ...
    velocity_output: dict[str, object] = {}
    for region_index, region_name in enumerate(REGION_NAMES):
        ...
        velocity_output[f"{root}/{_path_variant(velocity_signal_path)}/value"] = (
            with_attrs(
                raw,
        ...
        velocity_output[
            f"{root}/{_path_variant(velocity_filtered_path)}/value"
        ] = with_attrs(
            filtered,
            _velocity_region_attrs(region_name, "filtered", ("frame",)),
        )

    velocity_per_beat = _pack_region_per_beat_velocity_outputs(
        per_beat_paths,
        metrics,
        membership,
        vessel_name,
    )
    velocity_output.update(velocity_per_beat)
...
def _pack_region_per_beat_velocity_outputs(
    ...
    for region_index, region_name in enumerate(REGION_NAMES):
        ...
        result[f"{region_root}/{_path_variant(paths.velocity_signal)}/value"] = (
            with_attrs(
                region_raw,
